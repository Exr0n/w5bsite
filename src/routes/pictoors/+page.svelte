<script>
    ...
    let x = $derived(console.log(nat_widths))

    let open_pic = $state(null);
    let open_meta = $derived(open_pic !== null ? metadata[open_pic.split('/').at(-1).split('?').at(0)] : null)
</script>
<div class="hidden p-12 space-x-12 space-y-12 sm:flex-row lg:flex-row" />   <!-- force tailwind to compile all needed classes -->
...
<div class={`flex w-[100vw] space-x-${PHOTO_SPACING} p-${PHOTO_SPACING} sm:px-24 md:px-48 lg:px-72 min-h-[100vh] left-0 top-0 absolute bg-bg`}>
    <div class={`flex flex-col space-y-${PHOTO_SPACING} w-full lg:w-1/2`}>
        ...
        <button onclick={ () => { open_pic = path } } aria-label="Expand">
            <img src={path} class="object-contain" bind:naturalWidth={nat_widths[path]} bind:naturalHeight={nat_heights[path]} alt={meta.cap} />
        </button>
        {/each}
    </div>
...
<div
    ...
    tabindex="-1"
>
    <img src={open_pic} class="object-contain max-w-full max-h-full shadow-lg" onload={console.log(nat_heights)} alt={open_meta.cap}/>
